...
echo "PROD"  | while read CURRENT_ENV
do
...
do
    ...
    ### Do it!
    aws codepipeline create-pipeline --cli-input-json file://../pipelines/${PIPELINE}.json --region=$REGION --profile $PROFILE
    ...
    #MAC
    sed -i '' s/$ACCOUNT_NUMBER/_REPLACE_ACCOUNT_NUMBER_/g ../pipelines/${PIPELINE}.json #--# mac
    check_error
    sed -i '' s/$ENV/_REPLACE_ENV_/g ../pipelines/${PIPELINE}.json #--# mac
    check_error
    sed -i '' s/$GITHUB_CONN_UUID/_REPLACE-GITHUB_CONN_UUID_/g ../pipelines/${PIPELINE}.json
    check_error
    ...
    sed -i -e s/$ENV/_REPLACE_ENV_/g ../pipelines/${PIPELINE}.json #--# mac
    check_error
    sed -i -e s/$GITHUB_CONN_UUID/_REPLACE-GITHUB_CONN_UUID_/g ../pipelines/${PIPELINE}.json #--# check_error
    sed -i -e s/${CODESTAR_PROJECT_NAME}/_REPLACE_GITHUB_REPOID_/g ../pipelines/${PIPELINE}.json check_error
    sed -i -e s/${ORG_ID}/_REPLACE_GITHUB_ORGID_/g ../pipelines/${PIPELINE}.json
    check_error
